...
def convert_merged_vals_to_expression_matrix(merged_file, peak_folder='', output_file=None, orig_col="Normalized Tag Count",is_orig_merged=False, is_csrna=False):
    ...
    print('Number of duplicate indices (having same index name, '
    ...
    for col in peak_tissue_matrix.columns.values:
        ...
        try:
            #Different ways to load depends on which kind of merged files
            if is_orig_merged:
                orig_peaks_df = pd.read_csv(os.path.join(peak_folder, col), index_col=0, sep="\t")
                orig_peaks_df.index = orig_peaks_df.index.astype(int).astype(str) #convert to str
            elif is_csrna:
                orig_peaks_df = pd.read_csv(
                    os.path.join(peak_folder, col), index_col=0,
                    sep="\t")
            else:
        ...
        if not is_csrna:
            for i in orig_id[list(map(lambda x: ',' in x, orig_id))]:
                split_peaks = i.split(',')
                orig_peaks_df.loc[i, orig_col] = np.mean(
                    orig_peaks_df.loc[split_peaks,orig_col])
        else:
            # e.g. csRNA id with two peaks ['TSS-NW_020822423.1:289191-289341,+,TSS-NW_020822423.1:289521-289671,+'
            for i in orig_id[list(map(lambda x: x.count(',')>1, orig_id))]:
                split_peaks = []
                tmp = i.split(',')
                for ind in range(0, len(tmp), 2):
                    #print(ind)
                    #print(','.join(tmp[ind:ind + 2]))
                    split_peaks.append(','.join(tmp[ind:ind + 2]))
                orig_peaks_df.loc[i, orig_col] = np.mean(
                    orig_peaks_df.loc[split_peaks,orig_col])
        ...
        if len(orig_peaks_df.index.intersection(orig_id)) != len(orig_id):
            ...
            print(len(orig_peaks_df.index.intersection(orig_id)),len(orig_id))
            continue

        orig_peaks_df = orig_peaks_df.reindex(orig_id)
        vals = np.array(orig_peaks_df.loc[:, orig_col])
...
def wrap_distance_to_landmarks(peaks_file, landmark_file,
...
def create_anno_centric_df(peaks_df, tss_df, peak_tissue_matrix, peak_bin=(-1000, 100), anno_col='Nearest TSS',
    ...
    df[prom_col1] = [[]] * len(df)
    df[prom_col2] = [[]] * len(df)
    for i in tqdm.tqdm(peaks_df.groupby([anno_col])):
        curr_gene = i[0]
        if curr_gene == '':
            continue
...
def wrap_create_anno_centric(peaks_file, peaks_expression_file, tss_file,
    ...
    if f_save is not None:
        ...
        df.to_pickle(f_save.replace(".p","") + '.p')
...
def df_to_TSS_expression(f_in, f_mat_in, col_name=None, f_out=None):
    ...
    """
    df = pickle.load(open(f_in.replace(".p","") + ".p",'rb'))
...
def add_max_info(merged_df, expr_peaks, in_results=False, peaks_dir=None):
    ...
    # CHOconvert_merged_vals_to_expression_matrix
