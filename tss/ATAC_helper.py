...
rcParams['font.sans-serif'] = ['Arial','Verdana','Tahoma']
...
def run_heatmap(tss_bed, bigwig, outfile, dist=2000, numProc=6,
    ...
    if to_compute:
        cmd = 'computeMatrix reference-point -S {bw} -R {bd} -a {d} -b {d} -out {out} --numberOfProcessors {p} {args}'.format(
            bw=bigwig, bd=tss_bed, d=dist, out=outfile, p=numProc,
            args=computeMat_args)
        print(cmd)
        os.system(cmd)
    # !{cmd}
    if label is None:
    ...
    cmd = """plotHeatmap\
     ...
     --samplesLabel "{label}" \
     --plotTitle 'ATAC signal' {args}""".format(out=outfile,
                                                args=plotHeat_args,
                                                label=label)
    ...
    # if dispImage:
    #     return Image(filename=outfile + '.pdf')
    return
